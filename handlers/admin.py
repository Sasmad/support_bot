    ...
    try:
        ...
        if db_profile_access(uidown) >= 3:
            ...
            if len(args) == 2:
                ...
                await message.reply('⚠ Укажите аргументы команды\nПример: `/доступ 516712372 1`', parse_mode='Markdown')
                return
        else:
            await message.reply('⚠ У вас недостаточно прав для выполнения этой команды.', parse_mode='Markdown')
    except Exception as e:
    ...
    try:
        ...
        if db_profile_access(uidown) >= 2:
            ...
            if len(args) == 2:
                ...
                reason = args[1]
                if db_profile_exist(uid):
                    db_profile_updateone(uid, ban=1)
                ...
                await message.reply('⚠ Укажите аргументы команды\nПример: `/бан 51623722 Причина`', parse_mode='Markdown')
                return
        else:
            await message.reply('⚠ У вас недостаточно прав для выполнения этой команды.', parse_mode='Markdown')
    except Exception as e:
    ...
    try:
        ...
        if db_profile_access(uidown) >= 2:
            ...
            if len(args) == 1:
                uid = int(args[0])
                if db_profile_exist(uid):
                    db_profile_updateone(uid, ban=0)
                ...
                await message.reply('⚠ Укажите аргументы команды\nПример: `/разбан 516272834`', parse_mode='Markdown')
                return
        else:
            await message.reply('⚠ У вас недостаточно прав для выполнения этой команды.', parse_mode='Markdown')
    except Exception as e:
    ...
    try:
        ...
        if len(args) == 1:
            ...
            if db_profile_exist_usr(username):
                uid = db_profile_get_usrname(username, 'uid')
                await message.reply(f"🆔 {uid}")
            else:
                await message.reply("⚠ Этого пользователя *не* существует!", parse_mode='Markdown')
                return
        else:
            await message.reply('⚠ Укажите аргументы команды\nПример: `/айди nosemka`', parse_mode='Markdown')
            return
    except Exception as e:
        cid = message.chat.id
        await message.answer(f"{errormessage}", parse_mode='Markdown')
